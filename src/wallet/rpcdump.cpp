    ...
    while (file.good()) {
        ...
        if (fImportZKeys) {
            try {
                ...
                libzelcash::SpendingKey key = spendingkey.Get();
                libzelcash::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a Zelcash style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"zelcashaddress\"\n"
            "\nReveals the private key corresponding to 'zelcashaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"zelcashaddress\"   (string, required) The zelcash address for the private key\n"
            "\nResult:\n"
